...
# 内容首词 == on -> 自动放行,无需上手表。读不到 / 不是 on / 超过时效 -> 视为关闭
# (fail-safe:任何疑问都退回正常的上手表审批,绝不误放行)。
_AUTO_FOCUS_DEFAULT = os.path.expanduser(
    "~/Library/Mobile Documents/com~apple~CloudDocs/agent-focus.flag"
)
AUTO_FOCUS_FLAG = os.environ.get("AUTO_FOCUS_FLAG", "").strip() or _AUTO_FOCUS_DEFAULT
# 旗标多久算过期(分钟):防止「关闭专注」的自动化漏跑导致旗标一直停在 on。
# 0 = 不限时效(只认内容)。比如设 480 = 旗标写入超过 8 小时就当它已关。
try:
    AUTO_FOCUS_MAX_AGE_MIN = int(os.environ.get("AUTO_FOCUS_MAX_AGE_MIN", "0"))
except ValueError:
    AUTO_FOCUS_MAX_AGE_MIN = 0
# 自动模式下,命中危险清单(rm -rf / 强推 / sudo …)的操作怎么办:
#   allow(默认,按你的要求)= 危险操作也照样放行,完全无人值守;
#   notify                  = 危险操作仍推手表等你点(没点到按 TIMEOUT_DECISION 收尾),
#                             其余安全操作静默放行 —— 想「出门后基本全自动、但真要命的还是震我一下」选这个;
#   deny                    = 自动模式下危险操作直接拒绝。
AUTO_FOCUS_DANGER = os.environ.get("AUTO_FOCUS_DANGER", "allow").strip().lower()
if AUTO_FOCUS_DANGER not in ("allow", "notify", "deny"):
    AUTO_FOCUS_DANGER = "allow"


# macOS 专注侦测:除了 iCloud 旗标,还可直接读 macOS 同步过来的「专注」状态。
# iPhone 上把某个专注(默认名 Auto-CLI)开启 + 该专注「跨设备共享」打开后,Mac 的
# DoNotDisturb 数据库 Assertions.json 会出现一条活动记录;hook 直接读它 ->
# 该专注开着就自动放行。不需要 iOS 自动化、不耗网。
# 前提:运行 hook 的宿主 app(本机是 Claude.app)有「完全磁盘访问」才读得到该数据库;
# 读不到(无权限/Codex 从终端启动等)-> fail-safe 返回 False,退回正常审批。
# 触发自动模式的专注:逗号分隔,每项可写「显示名」或「modeIdentifier」(命中其一即可;
# 用 modeIdentifier 不受中英文名影响,最稳)。默认 4 个:Auto-CLI / 游戏 / 睡眠 / 勿扰。
# 新配置用 AUTO_FOCUS_NAMES(复数);旧的单数 AUTO_FOCUS_NAME 仍兼容(自动并入)。
...
AUTO_FOCUS_TRIGGERS = {
    t.strip()
    for t in (
        os.environ.get("AUTO_FOCUS_NAMES", _AUTO_FOCUS_DEFAULT_TRIGGERS)
...
def _icloud_flag_on():
    """iCloud 旗标文件内容首词 == on(大小写不敏感)且未过期 -> True。任何异常/缺失 -> False。"""
    try:
        with open(AUTO_FOCUS_FLAG, "r", encoding="utf-8-sig", errors="replace") as f:
            val = f.read(64).strip().lower()
    except Exception:
        return False
    if not val.startswith("on"):
        return False
    if AUTO_FOCUS_MAX_AGE_MIN > 0:
        try:
            age_min = (time.time() - os.path.getmtime(AUTO_FOCUS_FLAG)) / 60.0
            if age_min > AUTO_FOCUS_MAX_AGE_MIN:
                return False
        except Exception:
            return False
    return True
...
def mac_focus_active():
    """AUTO_FOCUS_TRIGGERS 里任一专注(按 modeIdentifier 或显示名)当前活动 -> True。读不到/没开/异常 -> False。"""
    if not AUTO_FOCUS_TRIGGERS:
        return False
    try:
        ...
        if not active_ids:
            return False
        # 1) 活动的 modeIdentifier 直接命中触发集
        if active_ids & AUTO_FOCUS_TRIGGERS:
            return True
        # 2) 或活动专注的显示名在触发集里(支持按名字配置)
        ...
        for key, cfg in modes.items():
            ...
            if mid in active_ids and m.get("name") in AUTO_FOCUS_TRIGGERS:
                return True
    except Exception:
        return False
    return False
...
def auto_focus_on():
    """自动模式总开关:iCloud 旗标=on,或 AUTO_FOCUS_NAME 专注处于活动 -> True。"""
    return _icloud_flag_on() or mac_focus_active()
...
_CDN = "https://cdn.jsdelivr.net/gh/LouringQ/agent-watch-approve"
_AGENT_PRESETS = {
    "claude": {
        "title": "🦀 Claude 待批准",
    ...
    "codex": {
        ...
        # 2026-06-15 官方 Codex 白色图标,777x243 透明横幅(匹配螃蟹尺寸),自托管 LouringQ/watch-icons 走 jsDelivr(Pushcut 可取,已锁 commit)。原 gpt-cat.png 见上注释。
        "image": "https://cdn.jsdelivr.net/gh/LouringQ/watch-icons@9a99e5d6cf1afbe74b255888005e68649d9f219d/codex-banner.png",
    },
}
...
def _publish_url(reply_topic, msg):
    ...
    设了 NTFY_TOKEN 时把 token 放进 ?access_token=(ntfy 官方支持的查询参数鉴权)。
    Pushcut 的按钮是云端「无自定义 header 的 GET」,带不了 Authorization 头,只能走
    URL 参数;这样请求就被认证 -> 限流按你 ntfy 账号算,躲开公共 ntfy.sh 对 Pushcut
    全球共享 IP 的匿名限流(就是之前点批准报的 429)。没设 token 则与旧版完全一致。
    """
    url = NTFY_BASE + urllib.parse.quote(reply_topic, safe="") + "/publish?message=" + msg
    if NTFY_TOKEN:
        url += "&access_token=" + urllib.parse.quote(NTFY_TOKEN, safe="")
    return url
...
def main():
    ...
    # 1.6) 自动模式总闸:iPhone「自动」专注开启(iCloud 旗标=on)-> 按 AUTO_FOCUS_DANGER 放行。
    #      放在最前面(配置缺失/网络都不影响)——自动放行是纯本地决策,不需要 Pushcut。
    if auto_focus_on():
        # .claude 下 settings/hooks/skills 等写操作会被 Claude Code 强制终端确认
        # (issue #41615),hook 返回 allow 也压不住。自动模式下也要先推一条提醒,
        # 否则你完全不知道终端在等你点(以前这类操作在这里被静默 allow,跳过了提醒)。
        if HOOK_EVENT != "PermissionRequest" and is_terminal_forced(tool_name, tool_input):
            try:
                send_notification(
                    make_opener(), "⚠️ 需去终端确认",
                    describe(tool_name, tool_input), with_actions=False, retries=5,
                )
            except Exception:
                pass
            emit("ask", "watch-approve: 自动模式,但该操作会被 Claude Code 强制终端确认,已推提醒到手表/手机。")
        _match = ""
        ...
        _danger = (
            ...
            or is_dangerous(_match)
            or is_protected_write(tool_name, tool_input, str(data.get("cwd") or ""))
        )
        if not _danger:
            emit("allow", "watch-approve: 自动模式(专注已开),非危险操作放行。")
        elif AUTO_FOCUS_DANGER == "allow":
            emit("allow", "watch-approve: 自动模式(专注已开),危险操作也放行。")
        elif AUTO_FOCUS_DANGER == "deny":
            emit("deny", "watch-approve: 自动模式,危险操作按配置拒绝。")
        # AUTO_FOCUS_DANGER == "notify":不 emit,继续往下走正常上手表流程。
